...
{
    public class BlogPostDeleteAction : IDataAction<string, bool>
    {
        public BlogPostDeleteAction(string slug)
        {
            Context = slug;
        }
        ...
        public bool Execute()
        {
            try
            {
                ...
                {
                    scope.UnitOfWork.RepositoryFor<BlogPost>()
                                    .Delete(new ExpressionValuePair<BlogPost, string>(x => x.Slug, Context));
                    scope.Commit();
                }
