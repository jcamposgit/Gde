...
{
    public class EventUpdateAction : IDataAction<Event, bool>
    {
        public EventUpdateAction(Event @event)
        {
            Context = @event;
        }
        ...
        public Event Context { get; private set; }
        public bool Execute()
        {
            try
            {
                ...
                {
                    scope.UnitOfWork.RepositoryFor<Event>().Save(Context);
                    scope.Commit();
                }
