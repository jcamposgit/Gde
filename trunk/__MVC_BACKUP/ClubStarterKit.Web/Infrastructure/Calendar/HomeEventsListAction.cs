...
{
    public class HomeEventsListAction : IDataAction<int, IEnumerable<Event>>
    {
        public int Context
        {
        ...
        {
            return new PagedDataCache<Event>().Only(x => x.StartTime >= DateTimeOffset.UtcNow)
                                              .WithPageSize(Constants.HomepageItemSize)
                                              .Sorted(x => x.StartTime)
                                              .CachedValue;
        }
    }
}